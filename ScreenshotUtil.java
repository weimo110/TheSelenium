...
public class ScreenshotUtil {
	...
	public static void getScreenshot(WebDriver driver, String file, String num) throws InterruptedException, IOException {
		if (num == null || "".equals(num)) {

			screenshot(driver, file);	
		} else {

			screenshotChildren(driver, file, num);
		}
	}
	
	private static void screenshot(WebDriver driver, String file) throws InterruptedException, IOException {
		...
		merge(files.toArray(new String[] {}), "png", file);

	}

	private static void screenshotChildren(WebDriver driver, String file, String num)
			throws InterruptedException, IOException {
		String js1 = "return document.body.children[" + num + "].clientHeight.toString()";
		String js1_result = ((JavascriptExecutor) driver).executeScript(js1) + "";
		String real_scroll_exist_children = "return document.body.children[" + num
				+ "].scrollHeight > document.body.children[" + num + "].clientHeight";
		...
		try {
			if (real_scroll_exist_children_b) {
				for (int i = 0; i < 20; i++) {
					int currentHeight = (i * height);
					String js = "document.body.children[" + num + "].scrollTo(0," + currentHeight + ");";
					((JavascriptExecutor) driver).executeScript(js);
					js1 = "return document.body.children[" + num
							+ "].scrollHeight.toString()+','+document.body.children[" + num + "].scrollTop.toString()";
					js1_result = ((JavascriptExecutor) driver).executeScript(js1) + "";
					int real_top = Integer.parseInt(js1_result.split(",")[1]);
					if (real_top == currentHeight) {
						files.add(screenshot(driver).getAbsolutePath());
	...
	private static File screenshot(WebDriver driver) throws InterruptedException, IOException {
		...
		Thread.sleep(1000);
		byte[] imageBytes = (byte[]) ((RemoteWebDriver) driver).getScreenshotAs(new OutputType<Object>() {
		...
		File file = File.createTempFile((new Random()).nextInt() + "", "png");
