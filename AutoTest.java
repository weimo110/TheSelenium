...
//		this.driver.manage().window().setSize(new Dimension(this.windowsWidth, this.windowsHeight));
	...
	private void showParameterMsg() {
		...
		System.err.println("请如下正确输入参数！");
		System.err.println("第一参数：环境区分（OLD：现环境，NEW：新环境)");
		System.err.println("第二参数：浏览器区分（I:internet explorer 11、E:Microsoft Edge、C:Google Chrome)");
		System.err.println("第三参数：方案名");
		System.err.println("第四参数：源名称");
	}
	...
	private void readConfigFile() throws Exception {
		...
		if (!file.exists()) {
			throw new Exception("配置文件「" + this.configFilePath + "」不存在！");
		}
	...
	private void endOperation() {
		...
		System.out.println("运行完成，请按Enter键关闭浏览器.");
